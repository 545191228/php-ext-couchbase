...
php_couchbase_stat_callback(libcouchbase_t handle,
	...
	if (LIBCOUCHBASE_SUCCESS != error || nkey == 0) {
		...
		zend_hash_update(Z_ARRVAL_P(node), (char *)key, nkey + 1, (void **)&v, sizeof(zval *), NULL);
	}

}
/* }}} */

/* {{{ static void php_couchbase_data_callback(...)
*/
static void php_couchbase_data_callback(libcouchbase_couch_request_t request,
    ...
    if (IS_STRING != Z_TYPE_P(ctx->rv)) {
        ZVAL_STRINGL(ctx->rv, (char *)bytes, nbytes, 1);
    } else {
        Z_STRLEN_P(ctx->rv) = spprintf(&(Z_STRVAL_P(ctx->rv)), 0, "%s%s", Z_STRVAL_P(ctx->rv), bytes);
    }
}
/* }}} */
...
create_new_link:
            ...
            php_ignore_value(libcouchbase_set_couch_data_callback(handle, php_couchbase_data_callback));
            php_ignore_value(libcouchbase_set_couch_complete_callback(handle, php_couchbase_complete_callback));
...
static void php_couchbase_view_impl(INTERNAL_FUNCTION_PARAMETERS, int oo) /* {{{ */ {
    ...
    if (oo) {
        zval *self = getThis();
        if(zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ss|a", &doc_name, &doc_name_len, &view_name, &view_name_len, &options) == FAILURE) {
            return;
        }
        res = zend_read_property(couchbase_ce, self, ZEND_STRL(COUCHBASE_PROPERTY_HANDLE), 1 TSRMLS_CC);
        if (ZVAL_IS_NULL(res) || IS_RESOURCE != Z_TYPE_P(res)) {
        ...
        if(zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "rss|a", &res, &doc_name, &doc_name_len, &view_name, &view_name_len, &options) == FAILURE) {
            return;
        }
    }
    {
...
# endif
        ...
        retval = libcouchbase_make_couch_request(couchbase_res->handle, (const void *)&ctx,
                                       uri, uri_len, query, query_len,
                                       (query_len > 1024)? LIBCOUCHBASE_HTTP_METHOD_POST : LIBCOUCHBASE_HTTP_METHOD_GET, 0/*chunked*/);
        efree(uri);
        smart_str_free(&buf);
        if (LIBCOUCHBASE_SUCCESS != retval) {
            php_error_docref(NULL TSRMLS_CC, E_WARNING,
                    "Failed to schedule couch request: %s", libcouchbase_strerror(couchbase_res->handle, retval));
            RETURN_FALSE;
        }
...
# endif
            ...
            ZVAL_FALSE(return_value);
            php_error_docref(NULL TSRMLS_CC, E_WARNING,
                    "Faild to execute view: %s", libcouchbase_strerror(couchbase_res->handle, ctx.res->rc));
        }
    }
}
/* }}} */
